...
class TypeScriptLanguageClient extends AutoLanguageClient {
  ...
  activate(...args) {
    ...
    // atom.config.onDidChange(
    //   `${this.getPackageName()}.linter.ignoredCodes`,
    //   () => this.sendConfiguration()
    // );
  }
  ...
  sendConfiguration() {
    ...
    try {
      ...
      // The connection will throw an exception if it's disposed or closed.
      console.error(`Error when sending configuration:`);
      console.error(err);
    }
  }
