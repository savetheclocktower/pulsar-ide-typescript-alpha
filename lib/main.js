...
class TypeScriptLanguageClient extends AutoLanguageClient {
  ...
  consumeLinterV2 (...args) {
    let enabled = atom.config.get('pulsar-ide-typescript-alpha.linter.enable');
    if (!enabled) return;

    return super.consumeLinterV2(...args);
  }

  getLinterSettings () {
    return (editor) => {
      let settings = {
        ...atom.config.get('pulsar-ide-typescript-alpha.linter')
      };
  ...
  getSymbolSettings () {
    return atom.config.get('pulsar-ide-typescript-alpha.symbols');
  }

  provideSymbols (...args) {
    let enabled = atom.config.get('pulsar-ide-typescript-alpha.symbols.enable');
    if (!enabled) return;

    return super.provideSymbols(...args);
  }
  ...
  getIgnoreIntentionsForLinterMessage ({ code, callback }, editor) {
    ...
    let intentions = [];
    let settings = this.getLinterSettings();
    let {
      ignoredCodes = [],
